...
func _ready():
	...
	for ally in get_tree().get_nodes_in_group("Allies"):
		...
		ally.connect("request_leader_change", self, "_on_request_leader_change")
		ally.connect("battle_engagement", self, "_on_battle_engagement")
...
func _unhandled_input(event):
	...
	if event.is_action_pressed("ui_fieldmenu") and ally_entities.size() > 0:
		...
		target_select = target_circle.instance()
...
func _on_battle_engagement(enter_exit):
	for ally in get_tree().get_nodes_in_group("Allies"):
		ally._FSM.set_flag("BATTLE_ENGAGED")
		if ally == ally_entities[active_idx]:
			ally._FSM.transition_to("BATTLE_ENGAGE", {enter_exit:null})

func _on_FieldUI_field_menu_closed():
	target_select.kill(true)
	field_menu = false
